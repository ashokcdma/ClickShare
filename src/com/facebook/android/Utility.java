...
public class Utility extends Application {
    ...
    private static int MAX_THNUM_IMAGE_DIMENSION = 255;
    ...
    public static byte[] scaleToImageThumbnail(Context context, Uri photoUri) throws IOException {
        InputStream is = context.getContentResolver().openInputStream(photoUri);
        BitmapFactory.Options dbo = new BitmapFactory.Options();
        dbo.inJustDecodeBounds = true;
        BitmapFactory.decodeStream(is, null, dbo);
        ...
        Bitmap srcBitmap;
        is = context.getContentResolver().openInputStream(photoUri);
        if (rotatedWidth > MAX_THNUM_IMAGE_DIMENSION || rotatedHeight > MAX_THNUM_IMAGE_DIMENSION) {
            float widthRatio = ((float) rotatedWidth) / ((float) MAX_THNUM_IMAGE_DIMENSION);
            float heightRatio = ((float) rotatedHeight) / ((float) MAX_THNUM_IMAGE_DIMENSION);
            float maxRatio = Math.max(widthRatio, heightRatio);
